...
トレイメニューの `BGM > 音量を上げる` / `BGM > 音量を下げる` から BGM の音量を 10% ずつ変更できます。
トレイメニューの `Pomodoro > 再起動` からアプリを再起動できます。
...
再通知の間隔は全OS共通で `pomowatcher_app/timer.py` の定数を編集:

```python
BREAK_REMINDER_INTERVAL_SEC = 5 * 60  # 休憩せず作業を続けた場合の再通知間隔
```
...
│   ↳ awaiting_break 中に作業を続けていると 5分ごとに同じ通知を再送
